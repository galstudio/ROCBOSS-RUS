...
Class doController extends base
{
    ...
    public function postTopic()
    {
        if ($this->checkPrivate(1) == true)
        {
            ...
            if (isset($_POST['title'], $_POST['msg'], $_POST['tag']) && $this->topicIn($_POST['msg']) != '')
            {
                ...
                {
                    $this->showMsg('Пожалуйста, введите пользовательские теги', 'error');
                }

                if (trim($_POST['title']) == '')
                {
                    $this->showMsg('Пожалуйста, введите название топика', 'error');
                }
                ...
                if ($insertTopicID > 0)
                {
                    ...
                    $this->showMsg('Топик успешно опубликован!', 'success', $insertTopicID);
                }
                else
                {
                    $this->showMsg('Опубликовать не удалось, попробуйте еще раз!', 'error');
                }
            }
            else
            {
                $this->showMsg('Пожалуйста, проверьте запись на правильный ввод текста', 'error');
            }
        }
        else
        {
            $this->showMsg('Вы не авторизовались в системе или были забанены, вы не можете публиковать новые сообщения!', 'error');
        }
    }
        ...
        if ($this->checkPrivate(1) == true)
        {
            ...
            if (isset($_POST['content'], $_POST['tid']) && $this->topicIn($_POST['content']) != '' && is_numeric($_POST['tid']) && $this->utils->getStrlen(trim($_POST['content'])) <= 250)
            {
                ...
                if ($this->app->db()->has('roc_topic', array('tid' => $tid)))
                {
                    ...
                    {
                        $this->showMsg('К сожалению, комментарии к этому топику отключены', 'error');
                    }
    ...
    public function doSign()
    {
        if ($this->checkPrivate() == true && $_POST['do'] == 'doSign')
        {
            ...
            {
                $this->showMsg('Вы уже отметили это сегодня - завтра не забудьте зайти', 'error');
            }
            else
            {
                ...
                $this->showMsg('Поздравляем, вы получили ' . $signScore . ' баллов', 'success', $signScore);
            }
        }
    }
    ...
    public function readWhisper()
    {
        if ($this->checkPrivate() == true)
        {
            if (isset($_POST['id']))
            {
                ...
                $this->showMsg('Успешно помечены как прочитанные!', 'success');
            }
        }
    }
    ...
    public function deliverWhisper()
    {
        if ($this->checkPrivate() == true)
        {
            if (isset($_POST['atuid'], $_POST['content']) && is_numeric($_POST['atuid']) && $this->utils->getStrlen(trim($_POST['content'])) <= 250 && strlen($_POST['content']) > 0)
            {
                ...
                {
                    $this->showMsg('Не ваше сообщение', 'error');
                }
                ...
                if ($this->app->db()->has('roc_user', array('uid' => $atuid)))
                {
                    ...
                    if ($myScore - $this->sys['scores_whisper'] >= 0)
                    {
                        ...
                        if ($WID > 0)
                        {
                            ...
                            $this->showMsg('Сообщение успешно отправлено, с вас списано '.$this->sys['scores_whisper'].' баллов', 'success');
                        }
                        else
                        {
                            $this->showMsg('Отправка не удалась, попробуйте еще раз', 'error');
                        }
                    }
                    else
                    {
                        $this->showMsg('У вас недостаточно баллов для отправки сообщения, за отправк списывается ' . $this->sys['scores_whisper'] . ' баллов', 'error');
                    }
                }
                else
                {
                    $this->showMsg('Пользователь не существует', 'error');
                }
            }
            else
            {
                $this->showMsg('Пожалуйста, проверьте правильность ввода данных', 'error');
            }
        }
        else
        {
            $this->showMsg('Вы не авторизовались в системе, недостаточно прав на операции', 'error');
        }
    }
    ...
    public function setEmail()
    {
        if ($this->checkPrivate() == true)
        {
            if (isset($_POST['email'], $_POST['password']))
            {
                ...
                {
                    $this->showMsg('Недопустимый адрес email', 'error', 1);
                }

                if ($this->app->db()->has('roc_user', array('email' => $email)))
                {
                    $this->showMsg('Такой email уже используется', 'error', 1);
                }
                ...
                )))
                {
                    ...
                    if ($resID > 0)
                    {
                        $this->showMsg('Настройка email прошла успешно', 'success');
                    }
                    else
                    {
                        $this->showMsg('Настройка email не удалось', 'error');
                    }
                }
                else
                {
                    $this->showMsg('Ошибка проверки пароля, пожалуйста, попробуйте снова', 'error', 1);
                }
            }
        }
    }
    ...
    public function setSignature()
    {
        if ($this->checkPrivate() == true)
        {
            if (isset($_POST['signature']))
            {
                ...
                if (empty($signature))
                {
                    $this->showMsg('Подпись не может быть пустой', 'error', 1);
                }

                if ($this->utils->getStrlen($signature) >= 32)
                {
                    $this->showMsg('Подпись не может быть больше, чем 32 знака', 'error', 1);
                }
                ...
                if ($resID > 0)
                {
                    $this->showMsg('Подпись успешно добавлена', 'success');
                }
                else
                {
                    $this->showMsg('Настройка подписи не удалась', 'error');
                }
            }
        }
    }
    ...
    public function setPassword()
    {
        if ($this->checkPrivate() == true)
        {
            if (isset($_POST['password'], $_POST['newPassword']))
            {
                ...
                {
                    $this->showMsg('Длина пароля не менее 6 знаков', 'error');
                }
                ...
                )) || $userOriPassword == '')
                {
                    ...
                    if ($resID > 0)
                    {
                        $this->showMsg('Новый пароль был установлен', 'success');
                    }
                    else
                    {
                        $this->showMsg('Не удалось установить новый пароль', 'error');
                    }
                }
                else
                {
                    $this->showMsg('Проверка пароля не удалось, попробуйте еще раз', 'error', 1);
                }
            }
        }
    }
    ...
    private function checkFloodTime($uid, $allowTime)
    {
        if ($this->app->db()->has('roc_user', array('uid' => $uid)))
        {
            ...
            {
                $this->showMsg('Вы слишком активны, похоже на спам. Пожалуйста, повторите попытку позже.', 'error');
            }
        }
        else
        {
            $this->showMsg('Недопустимая операция!', 'error');
        }
    }

    private function clientLoad()
    {
        # Инструменты библиотеки  инициализации
