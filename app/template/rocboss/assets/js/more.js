                        ...
                        if(t.find('.'+key)){
                            if(key == 'avatar'){
                                ...
                                if(value[0] != null) {
                                    ...
                                    for (i; i < value.length; i++) {
                                        ...
                                        if (login_groupid == 9) {
                                            if (login_uid != value[i].floorUid) {
                                                t.find('.'+key+' #floor-list-'+value[i].floorId).append('<span class=\"floor-admin right\"><a href=\"javascript:showFloorReply('+pid+',\'@'+value[i].floorUser+' \');\" title=\"Ответить\"><i class=\"icon icon-forward x1\"></i>Ответить</a><a class=\"delete-btn\" href=\"javascript:deleteFloor('+value[i].floorId+');\"><i class=\"icon icon-delete x1\"></i>Удалить</a></span>');
                                            } else {
                                                t.find('.'+key+' #floor-list-'+value[i].floorId).append('<span class=\"floor-admin right\"><a class=\"delete-btn\" href=\"javascript:deleteFloor('+value[i].floorId+');\"><i class=\"icon icon-delete x1\"></i>Удалить</a></span>');
                                            }
                                        } else {
                                            if (login_uid != value[i].floorUid && login_uid != 0) {
                                                t.find('.'+key+' #floor-list-'+value[i].floorId).append('<span class=\"floor-admin right\"><a href=\"javascript:showFloorReply('+pid+',\'@'+value[i].floorUser+' \');\" title=\"Ответить\"><i class=\"icon icon-forward x1\"></i>Ответить</a></span>');
                                    ...
                                    if (i >= 5) {
                                        t.find('.'+key).append('<div class=\"floor-more\"><a href=\"javascript:getMoreFloor('+pid+', 1);\"><i class=\"icon icon-unfold x1\"></i> Нажмите, чтобы загрузить больше комментариев</a></div>');
                                    } else {
                                        t.find('.'+key).append('<div class=\"floor-more\">Все комментарии уже загружены </div>');
                                    };
                                } else {
                                    t.find('.'+key).html('');
                                }
                                ...
                                if (uid == login_uid || login_groupid == 9) {
                                    t.find('.reply-admin').html('<a class=\"deleteReply\"><i class=\"icon icon-delete x1\"></i>Удалить</a>');
            ...
            if (h.substring(0, 7) == '#floor-') {
                ...
                if($('#d-reply-'+strArr[0]).length>0) {
                    if ($('#d-reply-'+strArr[0]+' #floor-list-'+strArr[1]).length>0) {
                        ...
                        alertMessage('Нажмите, чтобы загрузить больше комментариев');
                    }
                } else {
                    $('.get_more').trigger('click');
                }
