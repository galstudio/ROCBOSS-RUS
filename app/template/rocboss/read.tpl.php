    ...
    <div class="main-outlet container">
        <div class="content left">
            ...
            <li class="topic-view">
                ...
                <div class="topic-body">
                    ...
                    <div class="topicBottom">
                        <div class="right-admin x1">
                            {if $loginInfo['uid'] > 0}
                                <a class="praiseTopic btn-circle right" href="javascript:praiseTopic({$topicInfo.tid}, {$topicInfo.ispraise});" tip-title="{if $topicInfo['ispraise'] == 0}Нравится{else}Не нравится{/if}">
                            ...
                            {/if}
                            {if $loginInfo['uid'] > 0}
                                <a class="favorTopic btn-circle right" href="javascript:favorTopic({$topicInfo.tid}, {$topicInfo.isfavorite});" tip-title="{if $topicInfo['isfavorite'] == 0}В избранное{else}Из избранного{/if}">
                            ...
                            {/if}
                            {if $loginInfo['groupid'] == 9}
                                <a class="topTopic btn-circle right" href="javascript:topTopic({$topicInfo.tid}, {$topicInfo.istop});" tip-title="{if $topicInfo['istop'] == 0}Прикрепить{else}Открепить{/if}">
                                ...
                                <a class="lockTopic btn-circle right" href="javascript:lockTopic({$topicInfo.tid}, {$topicInfo.islock});" tip-title="{if $topicInfo['islock'] == 0}Отключить комментарии{else}Включить комментарии{/if}">
                            ...
                            {if $loginInfo['uid'] == $topicInfo['uid'] || $loginInfo['groupid'] == 9}
                                <a class="deleteTopic btn-circle right" href="javascript:deleteTopic({$topicInfo.tid});" tip-title="Удалить топик">
                        ...
                        <div class="topic-praise">
                            <span class="p-tips">Этим пользователям нравится топик</span>
                            {loop $topicInfo['praiseArray'] $c}
                            <a href="{$root}user/{$c.praiseUid}">
                                <img src="{$c.praiseAvatar}" title="Нравится {$c.praiseUsername}" alt="{$c.praiseUsername}" class="avatarC">
                            </a>
                            {/loop}
                        </div>
                        {else}
                                    ...
                                    {loop $reply['floor'] $floor}
                                        <div id="floor-list-{$floor.floorId}" class="floor-list">
                                            ...
                                            <span class="floor-admin right">
                                                ...
                                                <a href="javascript:showFloorReply({$floor.floorPid},'@{$floor.floorUser} ');" title="Ответить">
                                                    <i class="icon icon-forward x1"></i> Ответить
                                                </a>
                                                {/if}
                                    ...
                                    <div class="floor-more">
                                        {if count($reply['floor']) >= 5}
                                            <a href="javascript:getMoreFloor({$reply.pid}, 1);">
                                                <i class="icon icon-unfold x1"></i> Нажмите, чтобы загрузить больше комментариев
                                            </a>
                                        {else}
                                            Все комментарии уже загружены
                                        {/if}
                                    </div>
                                {/if}
                                </span>
                ...
                <div id="more">
                    <a href="javascript:;" class="get_more"><i class="icon icon-unfold x2"></i> Нажмите для загрузки  больше ответов </a>
                    <div class="reply-list" root-user-data="{$root}user/">
                        ...
                        <div class="reply-content">
                            ...
                            <div class="reply-bottom">
                                ...
                                {if $loginInfo['uid'] > 0}
                                    <a class="showFloorReply right"><i class="icon icon-forward x1"></i>Просмотр</a>
                                {/if}
